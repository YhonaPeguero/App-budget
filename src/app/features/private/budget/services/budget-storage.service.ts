...
import { budgetMock } from 'src/app/mock/budgetStorage.mock';
...
export class BudgetStorageService {
  ...
  showToast(transaction: TRANSACTION): void {
    this.toastr.success(
      `transaction realizada por un monto de ${transaction.amount.toLocaleString()} en fecha ${transaction.date} con la nota de ${transaction.note || 'sin comentario'}`,
